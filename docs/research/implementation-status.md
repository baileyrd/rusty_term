...
- [x] **Structured channel** — full-duplex JSON-RPC over a private OSC (`OSC 5379 ; <protocol> ; <json> ST`) with capability negotiation and graceful ANSI fallback (`l13/src/lib.rs`, feature `l13`). Hosts a complete **MCP** server (terminal introspection) plus **LSP/ACP** negotiation; reuses `rusty_lsp`'s JSON-RPC model + LSP types.
...
- [x] Structured side-channel: full-duplex JSON-RPC 2.0 over a private OSC (`OSC 5379 ; <protocol> ; <json> ST`), one message per OSC, replies written to the child's stdin via the response channel; unaware terminals ignore the OSC (graceful degradation). Feature-gated `l13`, runtime-agnostic. (`l13/src/lib.rs`, parser routing in `finish_osc`)
