...
**Status: done, including Windows taskbar (2026-07).** `Grid::set_progress` (`src/core/grid.rs`), parsed from ConEmu `OSC 9;4;st;pr` (`src/core/osc.rs`) — states normal/error/indeterminate/warning, percent clamped, state 0/unknown clears, cleared on RIS — and relayed to the host for TUI mode. The window's tab strip renders it as a ` 42%` / ` !42%` (error/paused) / ` …` (indeterminate) suffix on the tab label (`chrome_row`). On Windows, the focused tab's progress additionally mirrors onto the taskbar button via `ITaskbarList3` (`src/gui/taskbar.rs`) — `SetProgressState`/`SetProgressValue`, called once per frame alongside the existing `Window::set_title` sync. windows-sys 0.59 doesn't generate this COM interface even with `Win32_UI_Shell` enabled, so the ~10 vtable slots actually needed (of `ITaskbarList3`'s ~15) are hand-rolled — a stable, long-documented part of the shell ABI. Verified live: driving a real `new-tab`-then-`send-text` OSC 9;4 sequence through a running instance exercises the full `CoInitializeEx` → `CoCreateInstance` → vtable-call path repeatedly (every redraw) without a crash, and the instance stays responsive to further control-channel commands afterward — strong evidence the hand-rolled vtable offsets are correct. Not verified: the taskbar icon actually visibly showing the progress bar, since this environment can't sustain an interactive window session long enough to look (same limitation as G10/G30/G31).
